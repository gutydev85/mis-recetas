/* Mi Recetario — app.js v5.2 */
/* Correcciones: navegación por historial, botón atrás del celular, salir de modo cocinar */
...
// ===================== NAVIGATION HISTORY LOCK =====================
// Evita pushState duplicado cuando navegamos programáticamente (botón atrás, popstate, etc.)
let _navLock = false;
...
const ICONS = {
 ...
 chef: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M6 13.87A4 4 0 0 1 7.41 6a5.11 5.11 0 0 1 1.05-1.54 5 5 0 0 1 7.08 0A5.11 5.11 0 0 1 16.59 6 4 4 0 0 1 18 13.87V21H6ZM6 17h12"/><path d="M12 10v4"/></svg>',
 utensils: '<svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 2v1c0 1 2 1 2 2S3 6 3 7s2 1 2 2-2 1-2 2 2 1 2 2v1"/><path d="M20 2v1c0 1-2 1-2 2s2 1 2 2-2 1-2 2 2 1 2 2v1"/><path d="M12 2v9"/><path d="M12 22a5 5 0 0 1-5-5V7"/><path d="M12 22a5 5 0 0 0 5-5V7"/><path d="M8 7h8"/></svg>',
...
const Nav = {
 set(view) {
 // Empujar al historial del navegador para que el botón atrás del celular funcione
...
const Render = {
 ...
 '<b>' + State.categories.length + '</b> categoría' + (State.categories.length !== 1 ? 's' : '') + '';
 },
 ...
 '<button class="header-btn" onclick="event.stopPropagation();App.category.form(\'' + cat.id + '\')" title="Editar">' + ICONS.check.replace('14','16') + '</button>' +
 '</div>';
 ...
 '<div style="flex:1;min-width:0"><div class="name">Sin categoría</div>' +
...
row.innerHTML = (recipe.fotoPath ? '<img src="' + recipe.fotoPath + '" class="recipe-thumb" alt="">' : '<div class="recipe-thumb-placeholder">' + ICONS.chef + '</div>') +
 '<div class="info">' +
'<div class="title">' + escapeHtml(recipe.nombre) + '</div>' +
'<div class="meta">' + ICONS.clock + ' ' + catName + ' · ' + (recipe.tiempoMinutos || 0) + ' min</div>' +
'</div>' +
 '<div class="row-actions">' +
'<button class="fav-btn' + (recipe.favorito ? ' active' : '') + '" title="Favorito">' + ICONS.heart + '</button>' +
'</div>';
...
? '<img src="' + recipe.fotoPath + '" class="detail-photo" alt="">'
 ...
 const steps = parseLines(recipe.pasos);
 if (!steps.length) {
 DOM.detailSteps.innerHTML = '<p style="color:var(--text-tert);font-style:italic;">No hay pasos añadidos.</p>';
 return;
 }
...
'<button class="header-btn" onclick="App.timer.removeEditor(' + i + ')" title="Eliminar">✕</button>' +
'</div>' +
'</div>';
 }).join('');
 },
 ...
 const self = this;
 wrap.innerHTML = '<h3>Cronómetros de la receta</h3>' +
...
const CookMode = (() => {
 ...
 DOM.cookStepText.innerHTML = '<div class="cook-done"><div class="big-icon">🍲</div><h2>¡Listo!</h2><p>Todos los pasos completados. Buen provecho.</p><button class="btn-primary" onclick="App.cookMode.exit()" style="max-width:280px;margin-top:16px;">Volver a la receta</button></div>';
 AudioEngine.success();
 ...
 DOM.cookStepText.innerHTML = '<p>' + escapeHtml(stepText) + '</p>' + quickHtml;
 ...
 Storage.normalizeRecipe(recipe);
 if (!recipe.timers.length) {
 DOM.cookTimersList.innerHTML = '<p style="color:var(--text-dim);font-size:13px;">No hay cronómetros en esta receta</p>';
 return;
 }
...
'<button onclick="App.cookMode.startRecipeTimer(' + i + ')">▶</button>' +
...
if ('serviceWorker' in navigator) {
 ...
 // Manejar botón atrás del celular (popstate) para navegación dentro de la app
